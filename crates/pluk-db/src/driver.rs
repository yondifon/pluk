...
pub fn instrument_driver<D: Driver + 'static>(driver: D, ctx: crate::sql_log::SqlLogContext) -> InstrumentedDriver<D> {
...
impl<D: Driver + Send + Sync> Driver for InstrumentedDriver<D> {
    ...
    async fn explain(&self, sql: &str, params: &[serde_json::Value]) -> Result<QueryResult, DriverError> {
        crate::sql_log::run_with_sql_log_async(self.ctx.clone(), self.inner.explain(sql, params)).await
    }
    async fn list_tables(&self, schema: Option<&str>) -> Result<Vec<String>, DriverError> {
        let s = schema.map(|x| x.to_string());
        crate::sql_log::run_with_sql_log_async(self.ctx.clone(), self.inner.list_tables(s.as_deref())).await
    }
    async fn describe_table(&self, table: &str, schema: Option<&str>) -> Result<Vec<ColumnInfo>, DriverError> {
        let t = table.to_string();
        let s = schema.map(|x| x.to_string());
        crate::sql_log::run_with_sql_log_async(self.ctx.clone(), self.inner.describe_table(&t, s.as_deref())).await
    }
    async fn sample_table(&self, table: &str, limit: i64, schema: Option<&str>) -> Result<QueryResult, DriverError> {
        let t = table.to_string();
        let s = schema.map(|x| x.to_string());
        crate::sql_log::run_with_sql_log_async(self.ctx.clone(), self.inner.sample_table(&t, limit, s.as_deref())).await
    }
    async fn list_relationships(&self, table: Option<&str>, schema: Option<&str>) -> Result<Vec<RelationshipInfo>, DriverError> {
        let t = table.map(|x| x.to_string());
        let s = schema.map(|x| x.to_string());
        crate::sql_log::run_with_sql_log_async(self.ctx.clone(), self.inner.list_relationships(t.as_deref(), s.as_deref())).await
    }
    async fn search_schema(&self, term: &str, schema: Option<&str>) -> Result<Vec<SchemaSearchResult>, DriverError> {
        let term = term.to_string();
        let s = schema.map(|x| x.to_string());
        crate::sql_log::run_with_sql_log_async(self.ctx.clone(), self.inner.search_schema(&term, s.as_deref())).await
    }
    async fn table_stats(&self, table: &str, schema: Option<&str>) -> Result<TableStats, DriverError> {
        let t = table.to_string();
        let s = schema.map(|x| x.to_string());
        crate::sql_log::run_with_sql_log_async(self.ctx.clone(), self.inner.table_stats(&t, s.as_deref())).await
    }
    async fn list_schemas(&self) -> Result<Vec<String>, DriverError> {
    ...
    async fn get_full_schema(&self, schema: Option<&str>) -> Result<String, DriverError> {
        let s = schema.map(|x| x.to_string());
        crate::sql_log::run_with_sql_log_async(self.ctx.clone(), self.inner.get_full_schema(s.as_deref())).await
    }
    async fn test_connection(&self) -> Result<(), DriverError> { self.inner.test_connection().await }
    async fn close(&self) -> Result<(), DriverError> { self.inner.close().await }
}
...
{
    let Some(opts) = opts else { return fut.await; };
