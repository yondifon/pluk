...
mod tests {
    ...
    async fn cancellation_surfaces_as_cancelled_not_error() {
        ...
        let err = d
            .query("SELECT pg_sleep(10)", &[], Some(opts))
            .await
            .unwrap_err();
    ...
    async fn ssh_seam_rewrites_host_port() {
        ...
        impl crate::config::SshTunnelProvider for FakeTunnel {
            async fn open_tunnel(
                ...
                _remote_host: &str,
                _remote_port: u16,
            ) -> Result<crate::config::TunnelEndpoint, crate::error::DriverError> {
    ...
    async fn sqlite_remote_rejects_missing_ssh_host_with_typescript_message() {
        ...
        let err = crate::factory::create_driver(crate::factory::CreateDriverOpts::new(cfg))
            .await
            .unwrap_err();
    ...
    async fn sqlite_remote_exec_is_wired_and_timeout_propagates() {
        ...
        let opts = crate::types::QueryOpts {
            timeout_ms: Some(20),
            cancel: None,
        };
        let err = dw
            .driver
            .query("SELECT 1", &[], Some(opts))
            .await
            .unwrap_err();
        assert!(
            matches!(err, crate::error::DriverError::Timeout(20)),
            "got {:?}",
            err
        );
    }
    ...
    async fn sqlite_remote_exec_output_capped_at_one_million() {
        ...
        assert!(
            msg.contains("failed to parse sqlite3"),
            "large output should be capped then fail JSON parse, got: {}",
            msg
        );
    }
    ...
    async fn sqlite_remote_rejects_bind_params_with_injection_message() {
        ...
        let err = dw
            .driver
            .query("SELECT ?", &[serde_json::json!(1)], None)
            .await
            .unwrap_err();
        assert!(
            err.to_string()
