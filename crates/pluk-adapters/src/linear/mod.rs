...
fn linear_fields() -> Vec<ConfigField> {
    vec![
        ...
        ConfigField::new("team_key", "Default Team", FieldType::Text)
            .group("Defaults")
            .placeholder("ENG (optional, scopes list_issues)"),
    ]
}
...
fn create_issue_map() -> FieldMap {
    FieldMap::new(&["success", "issue"], &["issue.identifier", "issue.url"])
}
fn update_issue_map() -> FieldMap {
    FieldMap::new(
        &["success", "issue"],
        &["issue.identifier", "issue.state.name"],
    )
}
fn comment_map() -> FieldMap {
    FieldMap::new(&["success", "comment"], &["comment.url"])
}
...
pub fn thread_comments(nodes: Vec<Value>) -> Vec<Value> {
    ...
    for n in &nodes {
        ...
        if let Value::Object(ref mut m) = obj {
            m.insert("replies".to_string(), Value::Array(vec![]));
        }
        by_id.insert(id, obj);
    }
    ...
    for n in &nodes {
        ...
        let parent_exists = parent_id
            .map(|pid| by_id.contains_key(pid))
            .unwrap_or(false);
    ...
    if dedup.is_empty() && !roots.is_empty() {
        return roots;
    }
    dedup
}
...
impl Adapter for LinearAdapter {
    ...
    async fn test_connection(&self, conn: &Integration) -> Result<(), AdapterError> {
        ...
        if data.get("viewer").is_some() {
            Ok(())
        } else {
            Err(AdapterError::new("Linear test failed: no viewer"))
        }
    }
    ...
    fn register(
        ...
        const ISSUE_FIELDS: &str =
            "id identifier title state { name } assignee { name } priority url updatedAt";
        ...
        const COMMENT_FIELDS: &str =
            "id body url createdAt parentId resolvedAt user { name } botActor { name }";
        ...
        // list_issues
        {
            ...
            reg!(
                "list_issues",
                "List issues, optionally scoped to a team.",
                "read",
                {
                    let mut m = Map::new();
                ...
                },
                {
                    ...
                    move |args: &Value| {
                        format!(
                            ...
                            args.get("team").and_then(|v| v.as_str()).unwrap_or(&ft),
        ...
        reg!(
            "my_issues",
            "List the issues assigned to you — the starting point for \"what am I working on\". Open issues only unless include_done is set.",
            "read",
            {
                let mut m = Map::new();
            ...
            |args: &Value| format!(
                "my_issues include_done={} limit={}",
                args.get("include_done")
        ...
        reg!(
            ...
            "read",
            {
                ...
                m.insert(
                    "id".into(),
                    json!({"type":"string","description":"Issue id or identifier"}),
                );
        ...
        reg!(
            ...
            "read",
            {
                ...
                m.insert(
                    "query".into(),
                    json!({"type":"string","description":"Search term"}),
                );
                m.insert("limit".into(), json!({"type":"integer","minimum":1,"maximum":100,"default":25,"description":"Max issues to return"}));
                m.insert("only".into(), json!({"type":"array","items":{"type":"string"},"description": crate::projection::only_param_description(&["priority","url","ids"])}));
                m
        ...
        reg!(
            "list_comments",
            "Read an issue's comment thread, oldest first, with replies nested under the comment they answer. Use this to see the discussion and whether anyone responded.",
            "read",
            {
                let mut m = Map::new();
                m.insert(
                    "issue_id".into(),
                    json!({"type":"string","description":"Issue id or identifier (e.g. ENG-123)"}),
                );
            ...
            |args: &Value| format!(
                "list_comments {} limit={}",
                args.get("issue_id").and_then(|v| v.as_str()).unwrap_or(""),
            ...
            |args: Value, conn: &Integration| {
                ...
                Box::pin(async move {
                    ...
                    if issue.is_none() || issue.unwrap().is_null() {
                        return Err(AdapterError::new(format!(
                            "Issue \"{issue_id}\" not found."
                        )));
                    }
                    let issue_obj = issue.unwrap();
                    let identifier = issue_obj
                        .get("identifier")
                        .and_then(|v| v.as_str())
                        .unwrap_or("")
                        .to_string();
        ...
        reg!(
            "inbox",
            "Read your Linear notifications — replies to your comments, mentions, assignments and status changes, newest first. Unread only by default. Use this to find out who responded to you and where.",
            "read",
            {
                let mut m = Map::new();
            ...
            |args: Value, conn: &Integration| {
                ...
                Box::pin(async move {
                    ...
                    let unread_only = args
                        ...
                        .and_then(|v| v.as_bool())
                        .unwrap_or(true);
                    let first = if unread_only {
                        std::cmp::min(limit * 4, 200)
                    } else {
                        limit
                    };
                    ...
                    if unread_only {
                        nodes.retain(|n| {
                            n.get("readAt").is_none() || n.get("readAt").unwrap().is_null()
                        });
                    }
        ...
        reg!(
            ...
            |_: &Value| "list_teams".to_string(),
            |args: Value, conn: &Integration| {
                let (api_key, _) = linear_config(conn);
                Box::pin(async move {
                    let data =
                        linear_graphql(&api_key, "{ teams { nodes { id name key } } }", json!({}))
                            .await?;
                    let nodes = data
                        .get("teams")
        ...
        // list_states
        {
            ...
            reg!(
                "list_states",
                "List a team's workflow states (id, name, type). Use a state name with update_issue to move an issue.",
                "read",
                {
                    let mut m = Map::new();
                    m.insert("team".into(), json!({"type":"string","description":"Team key (e.g. ENG). Defaults to the integration's team if set."}));
        ...
        reg!(
            "list_projects",
            "List projects with their state, progress percent, and issue counts (total/completed). Optionally filter by name.",
            "read",
            {
                let mut m = Map::new();
        ...
        reg!(
            "project_updates",
            "Read a project's status-update log (the periodic updates with health on-track/at-risk/off-track), newest first. Use list_projects to find the project id.",
            "read",
            {
                let mut m = Map::new();
                m.insert(
                    "project_id".into(),
                    json!({"type":"string","description":"Project id (UUID) from list_projects"}),
                );
            ...
            |args: Value, conn: &Integration| {
                ...
                Box::pin(async move {
                    ...
                    if proj.is_none() || proj.unwrap().is_null() {
                        return Err(AdapterError::new(format!(
        ...
        reg!(
            "create_issue",
            "Create a new issue. Team by key or name, assignee by email or display name, state and labels by name.",
            "write",
            {
                let mut m = Map::new();
                ...
                m.insert(
                    "description".into(),
                    json!({"type":"string","description":"Issue description (markdown)"}),
                );
            ...
            |args: &Value| format!(
                "create_issue team={} \"{}\"",
                args.get("team").and_then(|v| v.as_str()).unwrap_or(""),
            ...
            |args: Value, conn: &Integration| {
                ...
                Box::pin(async move {
                    ...
                    if let Some(a) = args.get("assignee").and_then(|v| v.as_str()) {
                        let u = resolve_user(&api_key, a).await?;
                        input.as_object_mut().unwrap().insert(
                            "assigneeId".into(),
                            json!(u.get("id").and_then(|v| v.as_str()).unwrap_or("")),
                        );
                    }
                    if let Some(s) = args.get("state").and_then(|v| v.as_str()) {
                        let st = resolve_state(&api_key, &team_key, s).await?;
                        input.as_object_mut().unwrap().insert(
                            "stateId".into(),
                            json!(st.get("id").and_then(|v| v.as_str()).unwrap_or("")),
                        );
                    }
                    ...
                    if let Some(labels) = args.get("labels").and_then(|v| v.as_array()) {
                        let names: Vec<String> = labels
                            ...
                            .filter_map(|x| x.as_str().map(|s| s.to_string()))
                            .collect();
                        let ids = resolve_labels(&api_key, &names).await?;
                        input
                            .as_object_mut()
                            .unwrap()
                            .insert("labelIds".into(), json!(ids));
                    }
        ...
        reg!(
            "comment",
            "Add a comment to an issue, or reply in a thread by passing the comment id you are answering as parent_id. Get comment ids from list_comments.",
            "write",
            {
                let mut m = Map::new();
                m.insert(
            ...
            |args: &Value| if args.get("parent_id").is_some() {
                format!(
                    "comment {} reply-to={}",
                    args.get("issue_id").and_then(|v| v.as_str()).unwrap_or(""),
                    args.get("parent_id").and_then(|v| v.as_str()).unwrap_or("")
                )
            } else {
                format!(
                    "comment {}",
                ...
                Box::pin(async move {
                    ...
                    if let Some(pid) = parent_id {
                        ...
                        input
                            .as_object_mut()
                            .unwrap()
                            .insert("parentId".into(), Value::Null);
                    }
        ...
        reg!(
            "update_issue",
            "Update an issue — move it to another state, reassign or unassign, or change title, description, priority, estimate or labels. State by name, assignee by email or display name, labels by name.",
            "write",
            {
                let mut m = Map::new();
                m.insert(
                ...
                m.insert(
                    "state".into(),
                    json!({"type":"string","description":"New workflow state name, e.g. Done"}),
                );
                m.insert("assignee".into(), json!({"type":["string","null"],"description":"Assignee's email or display name; pass null to unassign"}));
                m.insert("priority".into(), json!({"type":"integer","minimum":0,"maximum":4,"description":"0 none, 1 urgent, 2 high, 3 normal, 4 low"}));
                m.insert(
                    "estimate".into(),
                    json!({"type":"integer","minimum":0,"description":"Estimate points"}),
                );
                m.insert(
                    "title".into(),
            ...
            |args: &Value| {
                let keys: Vec<String> = args
                    ...
                    .map(|o| {
                        o.keys()
                            .filter(|k| *k != "id" && args.get(*k).is_some())
                            .cloned()
                            .collect()
                    })
                    .unwrap_or_default();
                ...
                Box::pin(async move {
                    ...
                    if let Some(state) = args.get("state").and_then(|v| v.as_str()) {
                        ...
                        let team_key = issue
                            ...
                            .and_then(|k| k.as_str())
                            .ok_or_else(|| {
                                AdapterError::new(format!("Issue \"{id}\" not found."))
                            })?;
                        ...
                        .unwrap_or(false)
                    {
                        ...
                        if v.is_none() || v.unwrap().is_null() {
                            ...
                            let u = resolve_user(&api_key, s).await?;
                            input.insert(
                                "assigneeId".into(),
                                json!(u.get("id").and_then(|x| x.as_str()).unwrap_or("")),
                            );
                        }
                    }
                    if let Some(p) = args.get("priority") {
                        input.insert("priority".into(), p.clone());
                    }
                    if let Some(e) = args.get("estimate") {
                        input.insert("estimate".into(), e.clone());
                    }
                    if let Some(t) = args.get("title") {
                        input.insert("title".into(), t.clone());
                    }
                    if let Some(d) = args.get("description") {
                        input.insert("description".into(), d.clone());
                    }
                    if let Some(labels) = args.get("labels").and_then(|v| v.as_array()) {
                        let names: Vec<String> = labels
                            .iter()
                    ...
                    if input.is_empty() {
                        return Err(AdapterError::new(
                            "update_issue needs at least one field to change.",
                        ));
                    }
        ...
        reg!(
            "link_url",
            "Attach a URL to an issue — a pull request, build, or doc. URLs from a configured integration (GitHub, GitLab, Slack) become rich attachments that sync status back to the issue.",
            "write",
            {
                let mut m = Map::new();
                m.insert(
                ...
                m.insert(
                    "url".into(),
                    json!({"type":"string","format":"uri","description":"URL to attach"}),
                );
                m.insert(
                    "title".into(),
                    json!({"type":"string","description":"Link title shown on the issue"}),
                );
                m
            },
            |args: &Value| format!(
...
pub fn linear_adapters(store: Arc<pluk_store::Store>) -> Vec<Arc<dyn Adapter>> {
    vec![LinearAdapter::new(store)]
}
...
mod tests {
    ...
    fn issue_list_default_projection() {
        ...
        assert_eq!(
            out,
            json!([{"identifier":"ENG-42","title":"Fix","state":{"name":"In Progress"},"assignee":{"name":"Ada"},"updatedAt":"2026-08-01"}])
        );
    }
    #[test]
    fn issue_list_presets() {
        let node = json!({"identifier":"ENG-42","priority":2,"url":"https://x"});
        let out = project_value(
            node,
            Some(vec!["priority".into(), "url".into()]),
            &issue_list_map(),
        )
        .unwrap();
        assert_eq!(out, json!({"priority":2,"url":"https://x"}));
        let out2 = project_value(
            json!({"id":"iss-1"}),
            Some(vec!["ids".into()]),
            &issue_list_map(),
        )
    ...
    fn get_issue_default_and_presets() {
        ...
        assert_eq!(
            out,
            json!({"identifier":"ENG-42","title":"Fix","description":"Repro","state":{"name":"In Progress"},"assignee":{"name":"Ada"},"priority":2,"url":"https://x"})
        );
    ...
    fn inbox_and_comments_defaults() {
        ...
        assert_eq!(
            out,
            json!([{"type":"issueCommentMention","subtitle":"Ada replied","createdAt":"2026-08-01","actor":{"name":"Ada"},"issue":{"identifier":"ENG-42","title":"Fix"},"comment":{"body":"looks good"}}])
        );
        let comments = json!({"issue":"ENG-42","comments":[{"body":"hi","createdAt":"2026-08-01","user":{"name":"Ada"},"replies":[],"id":"c1","url":"https://x"}]});
        let out2 =
            project_value(comments, Some(vec!["refs".into()]), &list_comments_map()).unwrap();
        assert!(
            out2.get("comments").unwrap().as_array().unwrap()[0]
                .get("id")
                .is_some()
        );
    }
    ...
    async fn linear_auth_and_timeout_and_api_error() {
        // missing key
        let err = client::linear_graphql("", "query{}", json!({}))
            .await
            .unwrap_err();
        assert!(err.message.contains("missing"));
        // mock runner - api error
        client::set_linear_runner(Some(Arc::new(|_, _| {
            Box::pin(async { Err(AdapterError::new("Linear: Authentication required")) })
        })));
        let e = client::linear_graphql("k", "query{}", json!({}))
            .await
            .unwrap_err();
        assert!(e.message.contains("Authentication required"));
        // timeout simulation
        client::set_linear_runner(Some(Arc::new(|_, _| {
            Box::pin(async { Err(AdapterError::new("Linear API timed out after 20s")) })
        })));
        let e2 = client::linear_graphql("k", "query{}", json!({}))
            .await
            .unwrap_err();
        ...
        client::set_linear_runner(Some(Arc::new(move |q, v| {
            let c = c2.clone();
            Box::pin(async move {
                c.lock().unwrap().push((q.clone(), v.clone()));
                Ok(json!({"issues":{"nodes":[]}}))
            })
        })));
        let _ = client::linear_graphql(
            "lin_api_x",
            "query($first:Int!){issues}",
            json!({"first":25}),
        )
        .await
        .unwrap();
