...
pub type LinearRunner = Arc<dyn Fn(String, Value) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Value, AdapterError>> + Send>> + Send + Sync>;
...
pub async fn linear_graphql(api_key: &str, query: &str, variables: Value) -> Result<Value, AdapterError> {
    if api_key.is_empty() {
        return Err(AdapterError::new("Linear API key is missing. Set it in the integration config."));
    }
    ...
    json.get("data").cloned().ok_or_else(|| AdapterError::new("Linear API returned no data"))
}
