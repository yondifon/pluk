...
pub async fn resolve_team(api_key: &str, key_or_name: &str) -> Result<Value, AdapterError> {
    ...
    let exact: Vec<&Value> = nodes
        .iter()
        .filter(|t| {
            t.get("key")
                .and_then(|k| k.as_str())
                .map(|k| k.to_lowercase() == want)
                .unwrap_or(false)
                || t.get("name")
                    .and_then(|n| n.as_str())
    ...
    if exact.len() > 1 {
        ...
        return Err(AdapterError::new(format!(
            "Team \"{key_or_name}\" matches more than one team: {list}. Pass the exact team key."
        )));
    }
    ...
    Err(AdapterError::new(format!(
        "No team named \"{key_or_name}\". Known teams: {}.",
        cap(&known)
    )))
}
...
pub async fn resolve_user(api_key: &str, email_or_name: &str) -> Result<Value, AdapterError> {
    ...
    let filter = if by_email {
        json!({ "email": { "containsIgnoreCase": term } })
    } else {
        json!({ "name": { "containsIgnoreCase": term } })
    };
    ...
    if exact.len() > 1 {
        let list = exact
            ...
            .map(|u| {
                ...
                match u.get("email").and_then(|e| e.as_str()) {
            ...
            .join(", ");
        return Err(AdapterError::new(format!(
            "Assignee \"{email_or_name}\" matches more than one user: {list}. Pass a unique email or name."
        )));
    }
    let near: Vec<String> = nodes
        .iter()
        .map(|u| {
            let name = u.get("name").and_then(|n| n.as_str()).unwrap_or("?");
            match u.get("email").and_then(|e| e.as_str()) {
    ...
    if !near.is_empty() {
        return Err(AdapterError::new(format!(
            "No user matches \"{email_or_name}\". Near matches: {}.",
            cap(&near)
        )));
    }
    Err(AdapterError::new(format!(
        "No user matches \"{email_or_name}\"."
    )))
}
...
pub async fn resolve_state(
    ...
    if exact.len() > 1 {
        let list = exact
            ...
            .join(", ");
        return Err(AdapterError::new(format!(
            "State \"{name}\" matches more than one workflow state: {list}. Pass the exact state name."
        )));
    }
    let names: Vec<String> = nodes
        ...
        .collect();
    Err(AdapterError::new(format!(
        "No workflow state named \"{name}\" in team {team_key}. States: {}.",
        cap(&names)
    )))
}
    ...
    for raw in names {
        ...
        if exact.len() == 1 {
            ...
            let list = exact
                ...
                .join(", ");
            return Err(AdapterError::new(format!(
                "Label \"{raw}\" matches more than one label: {list}. Pass the exact label name."
            )));
        } else {
            let known: Vec<String> = nodes
                .iter()
                ...
                .collect();
            return Err(AdapterError::new(format!(
                "No label named \"{raw}\". Existing labels: {}.",
                cap(&known)
            )));
        }
    }
