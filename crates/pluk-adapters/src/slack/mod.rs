...
use client::{SlackConfig, resolve_channel, slack_config_from, slack_request};

pub use client::{TIMEOUT_MS, set_slack_runner};
...
fn channel_map() -> FieldMap {
    ...
    .with_preset(
        "details",
        Preset::paths(&["purpose", "num_members", "is_private"]),
    )
}
...
impl Adapter for SlackAdapter {
    ...
    fn tool_specs(&self) -> &[ToolSpec] {
        ...
        SPECS.get_or_init(|| {
            vec![
                ToolSpec::new(
                    "list_channels",
                    "List public channels in the workspace (id, name, topic).",
                    "read",
                ),
                ToolSpec::new(
                    "channel_history",
    ...
    async fn test_connection(&self, conn: &Integration) -> Result<(), AdapterError> {
        ...
        slack_request(&cfg, "auth.test", json!({}))
            .await
            .map(|_| ())
    }
    ...
    fn register(
        ...
        reg!(
            ...
            |_args: &Value| "list_channels".to_string(),
            |args: Value, cfg: &SlackConfig| {
                ...
                Box::pin(async move {
                    ...
                    let data = slack_request(
                        &cfg,
                        "conversations.list",
                        json!({"types":"public_channel","limit": limit}),
                    )
                    .await?;
        ...
        reg!(
            ...
            "write",
            {
                ...
                m.insert(
                    "text".into(),
                    json!({"type":"string","description":"Message text (markdown)"}),
                );
            ...
            |args: &Value| format!(
                "post_message {}",
                args.get("channel").and_then(|v| v.as_str()).unwrap_or("?")
            ),
            |args: Value, cfg: &SlackConfig| {
                let cfg = cfg.clone();
                Box::pin(async move {
...
pub fn slack_adapters(store: Arc<pluk_store::Store>) -> Vec<Arc<dyn Adapter>> {
    vec![SlackAdapter::new(store)]
}
...
mod tests {
    ...
    fn cfg_with_default(ch: Option<&str>) -> SlackConfig {
        ...
        if let Some(c) = ch {
            m.insert("default_channel".into(), json!(c));
        }
    ...
    fn projection_default_and_presets() {
        ...
        assert_eq!(
            out,
            json!([{"id":"C1","name":"general","topic":{"value":"hi"}}])
        );
        let out2 =
            project_value(json!([ch]), Some(vec!["details".into()]), &channel_map()).unwrap();
        assert_eq!(
            out2,
            json!([{"purpose":{"value":"p"},"num_members":3,"is_private":false}])
        );
        let msg = json!({"user":"U1","text":"hi","ts":"1","thread_ts":"1"});
        assert_eq!(
            project_value(msg.clone(), Some(vec!["*".into()]), &message_map()).unwrap(),
            msg
        );
        assert!(
            project_value(
