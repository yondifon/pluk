...
pub use client::{
    ExecResult, MAX_COMMAND_TIMEOUT_S, clear_test_executor, close_forward, list_forwards,
    open_forward, run_command, set_test_executor,
};
#[cfg(test)]
pub use client::{StubExecutor, reset_forwards_for_test};
pub use error::humanize_ssh_error;
pub use policy::{
    CommandCategory, CommandVerdict, evaluate_command, policy_summary, sanitize_working_dir,
};
pub use server::{SSH_AGENT_HINT, register_ssh_server, ssh_instructions, ssh_tool_specs};
...
fn ssh_fields() -> Vec<ConfigField> {
    let mut fields = vec![
        ConfigField::new("host", "Host", FieldType::Text)
            .group("Connection")
            .required()
            .placeholder("server.example.com or an ~/.ssh/config alias"),
        ConfigField::new("port", "Port", FieldType::Number)
            .group("Connection")
            .default_value(&serde_json::json!(22)),
        ConfigField::new("user", "User", FieldType::Text)
            .group("Connection")
            .placeholder("defaults to your local username"),
    ];
