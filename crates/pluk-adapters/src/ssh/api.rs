    ...
    match request.method.as_str() {
        "GET" => {
            ...
            Some(ApiResponse::json(
                200,
                &serde_json::json!({ "ok": true, "commands": commands }),
            ))
        }
        "POST" => {
            ...
            if name.is_empty() || command.is_empty() {
                return Some(ApiResponse::json(
                    400,
                    &serde_json::json!({ "ok": false, "error": "name and command required" }),
                ));
            }
            let input = pluk_store::SavedCommandInput {
            ...
            match store.create_saved_command(&input) {
                Ok(cmd) => Some(ApiResponse::json(
                    200,
                    &serde_json::json!({ "ok": true, "command": cmd }),
                )),
                Err(e) => {
                    let msg = e.to_string();
                    if msg.contains("UNIQUE") || msg.contains("unique") {
                        return Some(ApiResponse::json(
                            409,
                            &serde_json::json!({ "ok": false, "error": "A saved command with that name already exists." }),
                        ));
                    }
                    Some(ApiResponse::json(
                        500,
