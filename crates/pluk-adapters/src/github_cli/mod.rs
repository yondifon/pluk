...
use crate::projection::{FieldMap, Preset, apply_only, only_param_description};

pub use client::{
    GhConfig, GhRunResult, GhRunner, gh_command, gh_config, gh_cwd, gh_json, gh_text,
    humanize_gh_error, positional, repo_flag, resolve_repo, run_gh, set_gh_runner, test_gh,
};
...
fn issue_list_map() -> FieldMap {
    ...
    .with_preset(
        "authorship",
        Preset::paths(&["author", "createdAt", "updatedAt"]),
    )
}
fn issue_map() -> FieldMap {
    ...
    .with_preset(
        "metadata",
        Preset::paths(&["author", "createdAt", "updatedAt", "labels"]),
    )
}
...
fn file_map() -> FieldMap {
    ...
    .with_preset(
        "metadata",
        Preset::paths(&["name", "path", "sha", "size", "html_url", "type"]),
    )
}
fn files_map() -> FieldMap {
    ...
    .with_preset(
        "links",
        Preset::paths(&["blob_url", "raw_url", "contents_url"]),
    )
}
...
fn repo_map() -> FieldMap {
    ...
    .with_preset(
        "stats",
        Preset::paths(&["pushedAt", "stargazerCount", "forkCount"]),
    )
}
...
fn release_map() -> FieldMap {
    ...
    .with_preset(
        "metadata",
        Preset::paths(&["author", "createdAt", "publishedAt", "url"]),
    )
}
...
fn cwd_prop() -> (String, Value) {
    (
        "cwd".to_string(),
        prop_opt_string(
            "Working directory (e.g. a git worktree) to run gh in. Defaults to the integration's default working directory.",
        ),
    )
}
fn repo_prop() -> (String, Value) {
    (
        "repo".to_string(),
        prop_opt_string(
            "Repo as owner/repo. Defaults to the integration's default_repo; otherwise gh infers it from the cwd.",
        ),
    )
}
fn limit_prop() -> (String, Value) {
    (
        "limit".to_string(),
        json!({"type":"integer","description":"Max results to return","default":30}),
    )
}
fn only_prop(presets: &[&str]) -> (String, Value) {
    (
        "only".to_string(),
        json!({"type":"array","items":{"type":"string"},"description": only_param_description(presets)}),
    )
}
...
pub fn github_cli_tools(cfg: GhConfig) -> Vec<ActionTool> {
    ...
    // list_issues
    {
        ...
        tools.push(
            ActionTool::new(
                "list_issues",
                "List issues in a repo, newest first (excludes pull requests).",
                ActionCategory::Read,
            )
            .schema(props)
            .detail_fn(move |args| {
                let repo = extract_str(args, "repo")
                    .unwrap_or_else(|| cfg_detail.default_repo.clone().unwrap_or("?".to_string()));
    ...
    // get_issue
    {
        ...
        props.insert(
            "number".to_string(),
            json!({"type":"integer","description":"Issue number"}),
        );
        props.insert(only_prop(&["metadata"]).0, only_prop(&["metadata"]).1);
        tools.push(
            ActionTool::new(
                "get_issue",
                "Get a single issue by number, with its comments.",
                ActionCategory::Read,
            )
            .schema(props)
            .detail_fn(move |args| {
    ...
    // get_pull_request
    {
        ...
        props.insert(
            "number".to_string(),
            json!({"type":"integer","description":"PR number"}),
        );
        props.insert(
            only_prop(&["branch", "metadata"]).0,
            only_prop(&["branch", "metadata"]).1,
        );
        tools.push(
            ActionTool::new(
                "get_pull_request",
                "Get a single pull request by number (title, body, state, mergeability).",
                ActionCategory::Read,
            )
            .schema(props)
            .detail_fn(move |args| {
    ...
    // pr_files
    {
        ...
        tools.push(
            ActionTool::new(
                "pr_files",
                "List the changed files (with patches) for a pull request.",
                ActionCategory::Read,
            )
            .schema(props)
            .detail_fn(move |args| {
            ...
            .run({
                ...
                move |args, _| {
                    ...
                    async move {
                        ...
                        let (owner, repo_name) = resolve_repo(&c, repo.as_deref())?;
                        let url =
                            format!("repos/{owner}/{repo_name}/pulls/{n}/files?per_page={limit}");
                        let a = vec![
                            "api".to_string(),
                            "--method".to_string(),
                            "GET".to_string(),
    ...
    // search_code
    {
        ...
        props.insert(
            "query".to_string(),
            json!({"type":"string","description":"Code search query"}),
        );
        props.insert(limit_prop().0, limit_prop().1);
        props.insert(
            only_prop(&["ranking", "matches"]).0,
            only_prop(&["ranking", "matches"]).1,
        );
        tools.push(
            ActionTool::new(
                "search_code",
            ...
            .run({
                ...
                move |args, _| {
                    ...
                    async move {
                        ...
                        let only = extract_only(&args);
                        let url =
                            format!("search/code?q={}&per_page={limit}", urlencoding::encode(&q));
                        let a = vec![
                            "api".to_string(),
                            "--method".to_string(),
                            "GET".to_string(),
                        ...
                        let cmd = gh_command(&c, &a);
                        let data = gh_json(&c, a, cwd.as_deref()).await?;
                        let items = data.get("items").cloned().unwrap_or_else(|| {
                            if data.is_array() {
                                data.clone()
                            } else {
                                json!([])
    ...
    // get_file
    {
        ...
        props.insert(
            "path".to_string(),
            json!({"type":"string","description":"File path within the repo"}),
        );
        ...
        tools.push(
            ActionTool::new(
                "get_file",
                "Get a file's contents at an optional ref (branch/tag/sha).",
                ActionCategory::Read,
            )
            .schema(props)
            .detail_fn(move |args| {
            ...
            .command_fn({
                ...
                move |args, _| {
                    ...
                    let (owner, repo_name) = resolve_repo(&c, repo.as_deref())
                        .unwrap_or(("?".to_string(), "?".to_string()));
                    let ref_q = r
                        .map(|v| format!("?ref={}", urlencoding::encode(&v)))
                        .unwrap_or_default();
                    let url = format!(
    ...
    // commit_status
    {
        ...
        props.insert(
            "ref".to_string(),
            json!({"type":"string","description":"Branch, tag, or commit sha"}),
        );
        props.insert(only_prop(&["links"]).0, only_prop(&["links"]).1);
        tools.push(
            ActionTool::new(
                "commit_status",
                "Get the combined commit status and check-runs for a ref (CI state).",
                ActionCategory::Read,
            )
            ...
            .command_fn({
                ...
                move |args, _| {
                    ...
                    let (owner, repo_name) = resolve_repo(&c, repo.as_deref())
                        .unwrap_or(("?".to_string(), "?".to_string()));
                    let url = format!(
                        "repos/{owner}/{repo_name}/commits/{}/status",
                ...
                move |args, _| {
                    ...
                    async move {
                        ...
                        let a1 = vec![
                            ...
                            "--method".to_string(),
                            "GET".to_string(),
                            format!("repos/{owner}/{repo_name}/commits/{enc}/status"),
                        ];
                        let a2 = vec![
                            "api".to_string(),
                            "--method".to_string(),
                            "GET".to_string(),
                            format!("repos/{owner}/{repo_name}/commits/{enc}/check-runs"),
                        ];
    ...
    // get_repo
    {
        ...
        tools.push(
            ActionTool::new(
                "get_repo",
                "Get repository metadata (owner, description, default branch, visibility).",
                ActionCategory::Read,
            )
            .schema(props)
            .detail_fn(move |args| {
    ...
    // list_releases
    {
        ...
        tools.push(
            ActionTool::new(
                "list_releases",
                "List releases in a repo, newest first.",
                ActionCategory::Read,
            )
            .schema(props)
            .detail_fn(move |args| {
    ...
    // get_release
    {
        ...
        props.insert(
            "tag".to_string(),
            json!({"type":"string","description":"Release tag, e.g. v1.2.3"}),
        );
        props.insert(only_prop(&["metadata"]).0, only_prop(&["metadata"]).1);
        tools.push(
            ActionTool::new(
                "get_release",
                "Get a single release by tag (body, assets, draft/prerelease state).",
                ActionCategory::Read,
            )
            .schema(props)
            .detail_fn(move |args| {
    ...
    // add_comment
    {
        ...
        props.insert(
            "number".to_string(),
            json!({"type":"integer","description":"Issue or PR number"}),
        );
        props.insert(
            "body".to_string(),
            json!({"type":"string","description":"Comment body (markdown)"}),
        );
        tools.push(
            ActionTool::new(
                "add_comment",
                "Add a comment to an issue or pull request (PRs share the issue comment endpoint).",
    ...
    // create_issue
    {
        ...
        props.insert(
            "title".to_string(),
            json!({"type":"string","description":"Issue title"}),
        );
        props.insert(
            "body".to_string(),
            json!({"type":"string","description":"Issue body (markdown)"}),
        );
        tools.push(
            ActionTool::new("create_issue", "Create a new issue.", ActionCategory::Write)
                .schema(props)
                .detail_fn(move |args| {
                    let repo = extract_str(args, "repo").unwrap_or_else(|| {
                        cfg_detail.default_repo.clone().unwrap_or("?".to_string())
                    });
    ...
    // create_pull_request
    {
        ...
        props.insert(
            "title".to_string(),
            json!({"type":"string","description":"PR title"}),
        );
        ...
        props.insert(
            "body".to_string(),
            json!({"type":"string","description":"PR body (markdown)"}),
        );
        props.insert(
            "draft".to_string(),
            json!({"type":"boolean","description":"Open as a draft"}),
        );
        tools.push(
    ...
    // review_pull_request
    {
        ...
        tools.push(
            ActionTool::new(
                "review_pull_request",
                "Submit a review on a pull request: approve, comment, or request changes.",
                ActionCategory::Write,
            )
            .schema(props)
            .detail_fn(move |args| {
    ...
    // create_release
    {
        ...
        props.insert(
            "title".to_string(),
            json!({"type":"string","description":"Release title (defaults to the tag)"}),
        );
        props.insert(
            "notes".to_string(),
            json!({"type":"string","description":"Release notes (markdown)"}),
        );
        props.insert(
            "draft".to_string(),
            json!({"type":"boolean","description":"Create as a draft"}),
        );
        props.insert(
            "prerelease".to_string(),
            json!({"type":"boolean","description":"Mark as a prerelease"}),
        );
        tools.push(
            ActionTool::new(
                "create_release",
                "Publish a release for a tag (draft or prerelease optional).",
                ActionCategory::Write,
            )
            .schema(props)
            .detail_fn(move |args| {
            ...
            .command_fn({
                ...
                move |args, _| {
                    ...
                    if let Some(t) = title
                        && !t.is_empty()
                    {
                        a.extend(vec!["--title".to_string(), t]);
                    }
                    if let Some(n) = notes
                        && !n.is_empty()
                    {
                        a.extend(vec!["--notes".to_string(), n]);
                    }
                    if draft {
                        a.push("--draft".to_string());
                    }
                    if prerelease {
                        a.push("--prerelease".to_string());
                    }
                    gh_command(&c, &a)
                }
            })
...
mod tests {
    ...
    fn set_fake(
        calls: Arc<Mutex<Vec<Calls>>>,
        responder: Arc<dyn Fn(&Vec<String>) -> (i32, String, String) + Send + Sync>,
    ) {
        let c = calls.clone();
    ...
    fn gh_config_honours_fields() {
        let cfg = gh_cfg(
            json!({"gh_bin":" ~/bin/gh ", "default_repo":"acme/app", "default_cwd":"/wt", "timeout_seconds":10}),
        );
    ...
    fn resolve_repo_contract() {
        let cfg = gh_cfg(json!({"default_repo":"acme/app"}));
        assert_eq!(
            resolve_repo(&cfg, Some("other/repo")).unwrap(),
            ("other".to_string(), "repo".to_string())
        );
        assert_eq!(
            resolve_repo(&cfg, None).unwrap(),
    ...
    async fn run_gh_forwards_cwd() {
        ...
        set_fake(
            calls.clone(),
            Arc::new(|_| (0, "[]".to_string(), "".to_string())),
        );
        crate::github_cli::client::run_gh(
            &cfg,
    ...
    fn list_pull_requests_builds_args() {
        ...
        let t = tools
            .into_iter()
            .find(|x| x.name == "list_pull_requests")
            .unwrap();
        let cmd_fn = t.command.unwrap();
        let cmd = cmd_fn(
            &json!({"repo":"acme/app","state":"open","limit":30}),
            &Map::new(),
        );
        ...
        let args = vec![
            ...
            "30".to_string(),
            "--json".to_string(),
            "number,title,state,headRefName,baseRefName,author,createdAt,updatedAt".to_string(),
        ];
    ...
    async fn get_issue_passes_number_and_cwd() {
        ...
        let _ = (t.run)(
            json!({"repo":"acme/app","number":12,"cwd":"/wt/feature"}),
            Map::new(),
        )
        .await
        .unwrap();
    ...
    async fn nonzero_surfaces_stderr() {
        ...
        set_fake(
            calls,
            Arc::new(|_| (1, "".to_string(), "not authorized".to_string())),
        );
        let tools = github_cli_tools(cfg);
        let t = tools
            .into_iter()
    ...
    async fn worktree_pr_creation_defaults() {
        ...
        set_fake(
            calls.clone(),
            Arc::new(|_| (0, "ok".to_string(), "".to_string())),
        );
        let tools = github_cli_tools(cfg);
        let t = tools
            .into_iter()
            .find(|x| x.name == "create_pull_request")
            .unwrap();
        let _ = (t.run)(json!({"cwd":"/wt/feature","title":"Add auth"}), Map::new())
            .await
            .unwrap();
        assert_eq!(calls.lock().unwrap()[0].cwd, "/wt/feature");
        assert_eq!(
            calls.lock().unwrap()[0].args,
            vec!["pr", "create", "--title", "Add auth"]
        );
        clear_fake();
    }
    #[tokio::test]
    ...
    async fn api_backed_pr_files_resolves_repo() {
        ...
        let _ = (t.run)(json!({"repo":"acme/app","number":7,"limit":30}), Map::new())
            .await
            .unwrap();
        assert_eq!(
            calls.lock().unwrap()[0].args,
            vec![
                ...
                "--method",
                "GET",
                "repos/acme/app/pulls/7/files?per_page=30"
            ]
        );
        clear_fake();
        let tools2 = github_cli_tools(cfg);
        let t2 = tools2.into_iter().find(|x| x.name == "pr_files").unwrap();
        let err = (t2.run)(json!({"number":7}), Map::new())
            .await
            .expect_err("should err no repo");
    ...
    async fn release_tools_cover_list_view_create() {
        ...
        let _ = (lr.run)(json!({"repo":"acme/app","limit":30}), Map::new())
            .await
            .unwrap();
        ...
        set_fake(
            calls.clone(),
            Arc::new(|_| {
                (0,r#"{"tagName":"v1.2.3","name":"v1.2.3","body":"b","isDraft":false,"isPrerelease":false,"assets":[]}"#.to_string(),"".to_string())
            }),
        );
        let tools = github_cli_tools(gh_cfg(json!({})));
        let gr = tools.iter().find(|x| x.name == "get_release").unwrap();
        let _ = (gr.run)(json!({"repo":"acme/app","tag":"v1.2.3"}), Map::new())
            .await
            .unwrap();
        assert_eq!(
            calls.lock().unwrap()[0].args,
            vec![
                ...
                "acme/app",
                "--json",
                "tagName,name,body,isDraft,isPrerelease,author,createdAt,publishedAt,assets,url"
            ]
        );
        clear_fake();
        let calls = Arc::new(Mutex::new(Vec::new()));
        ...
        let _ = (cr.run)(
            json!({"repo":"acme/app","tag":"v1.2.3","title":"v1.2.3","notes":"Notes","draft":true}),
            Map::new(),
        )
        .await
        .unwrap();
        assert_eq!(
            calls.lock().unwrap()[0].args,
            vec![
                "release", "create", "v1.2.3", "--repo", "acme/app", "--title", "v1.2.3",
                "--notes", "Notes", "--draft"
            ]
        );
        clear_fake();
    ...
    async fn tag_flag_refused() {
        ...
        let err = (t.run)(json!({"repo":"acme/app","tag":"--draft"}), Map::new())
            .await
            .expect_err("should err");
    ...
    async fn test_connection_guidance() {
        ...
        set_fake(
            calls.clone(),
            Arc::new(|_| (1, "".to_string(), "Please log in first".to_string())),
        );
        let err = adapter
            .test_connection(&integ)
            .await
    ...
    fn humanize_points_to_login() {
        assert!(
            humanize_gh_error(&AdapterError::new("gh executable not found: /nope/gh"))
                .contains("gh auth login")
        );
        assert!(
            humanize_gh_error(&AdapterError::new(
                "gh pr list failed (exit 1): Please log in"
            ))
            .contains("gh auth login")
        );
    }
    ...
    fn adapter_exposes_surface() {
        ...
        let by_name: std::collections::HashMap<&str, &crate::tool_spec::ToolSpec> =
    ...
    async fn get_repo_projection() {
        ...
        set_fake(
            calls.clone(),
            Arc::new(|_| {
                (0, r#"{"name":"app","owner":{"login":"acme"},"description":"d","url":"u","defaultBranchRef":{"name":"main"},"isPrivate":false,"stargazerCount":2,"pushedAt":"2026-01-01","forkCount":1}"#.to_string(),"".to_string())
            }),
        );
        let tools = github_cli_tools(cfg);
        let repo = tools.into_iter().find(|x| x.name == "get_repo").unwrap();
        let res = (repo.run)(json!({"repo":"acme/app"}), Map::new())
            .await
            .unwrap();
        let val = match res {
            ActionOutput::WithCommand { value, .. } => value,
            ActionOutput::Value(v) => v,
        };
        assert_eq!(
            val,
            json!({"name":"app","owner":{"login":"acme"},"description":"d","url":"u","defaultBranchRef":{"name":"main"},"isPrivate":false})
        );
        let res2 = (repo.run)(json!({"repo":"acme/app","only":["stats"]}), Map::new())
            .await
            .unwrap();
        let val2 = match res2 {
            ActionOutput::WithCommand { value, .. } => value,
        ...
        let res3 = (repo.run)(json!({"repo":"acme/app","only":["*"]}), Map::new())
            .await
            .unwrap();
        let val3 = match res3 {
            ActionOutput::WithCommand { value, .. } => value,
        ...
        let err = (repo.run)(json!({"repo":"acme/app","only":["missing"]}), Map::new())
            .await
            .expect_err("should err unknown");
    ...
    async fn list_issues_default_and_preset_and_star() {
        ...
        set_fake(
            calls.clone(),
            Arc::new(|_| {
                (0, r#"[{"number":1,"title":"T","state":"open","labels":[],"author":{"login":"a"},"createdAt":"c","updatedAt":"u"}]"#.to_string(),"".to_string())
            }),
        );
        let tools = github_cli_tools(cfg);
        ...
        let v = match res {
            ActionOutput::WithCommand { value, .. } => value,
            ActionOutput::Value(x) => x,
        };
        assert_eq!(
            v,
            json!([{"number":1,"title":"T","state":"open","labels":[]}])
        );
        let res2 = (t.run)(json!({"repo":"acme/app","only":["authorship"]}), Map::new())
            .await
            .unwrap();
        let v2 = match res2 {
            ActionOutput::WithCommand { value, .. } => value,
        ...
        assert_eq!(
            v2,
            json!([{"author":{"login":"a"},"createdAt":"c","updatedAt":"u"}])
        );
        let res3 = (t.run)(json!({"repo":"acme/app","only":["*"]}), Map::new())
            .await
            .unwrap();
        let v3 = match res3 {
            ActionOutput::WithCommand { value, .. } => value,
        ...
        assert_eq!(
            v3,
            json!([{"number":1,"title":"T","state":"open","labels":[],"author":{"login":"a"},"createdAt":"c","updatedAt":"u"}])
        );
        clear_fake();
    }
