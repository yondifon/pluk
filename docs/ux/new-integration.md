...
- **Adapters that run commands get one extra step.** SSH and GitHub CLI set `runs_commands(true)` (`crates/pluk-adapters/src/ssh/mod.rs:77`, `crates/pluk-adapters/src/github_cli/mod.rs:1573`). For those, step 4 becomes two screens: "Choose what the agent can do" (tools) and "What it's allowed to run" (the existing allow/deny/ask rules), instead of both stacked in one long card as they are in today's modal.
